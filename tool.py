...
color_map = [(119, 172, 48), (0, 119, 255), (76, 175, 80), (0, 0, 255), (188, 18, 162), (42, 42, 165),
             (180, 105, 255), (128, 128, 128)]
weed_pixs_color_map = [(119, 172, 48), (0, 119, 255), (76, 175, 80), (0, 0, 255), (188, 18, 162), (42, 42, 165),
                       (180, 105, 255), (128, 128, 128)]
class_name = ["0", "1", "2", "3"]
...
# 去除苗或杂草
def separate_2dif(image, target_ayy, if_draw=True, if_weedpixs=False, if_differentiation=True, green_up_limit=0.2):
    ...
    if if_draw:
        ...
        #     else:
        if if_differentiation:
            img, weed_midpoints = findcontours_M(image, binary, if_draw=False)
        else:
            img, weed_midpoints = findcontours_M(image, binary, if_draw=if_draw)
        for i in weed_midpoints_1:
            weed_midpoints.append(i)
        if if_weedpixs:
            green_percentage = cv2.countNonZero(binary)  # 计算二值化图像非零像素数量
            weed_pixs = green_percentage / (binary.shape[0] * binary.shape[1])
            if weed_pixs >= green_up_limit:
                if_Warning_Area = True
            return img, weed_midpoints, maize_midpoints, weed_pixs, if_Warning_Area
        return img, weed_midpoints, maize_midpoints, weed_pixs, if_Warning_Area
...
def midpoints_plot(image, seeding_midpoints_arr, weed_midpoints_arr, if_weed=True, if_seeding=True):
    ...
    if if_weed:
        for (cls, (x, y)) in weed_midpoints_arr:  # 绘制草点
            cv2.circle(mask, (int(x), int(y)), 6, color=weed_pixs_color_map[cls], thickness=-1)
    if if_seeding:
