...
color_map = [(125, 0, 0), (0, 125, 0), (0, 0, 125), (50, 125, 255)]
class_name = ["Maize", "weed_1", "weed_2", "weed_3"]
...
def findcontours_M(image, binary, if_draw=True):
    ...
    for c in cnts:
        ...
        weed_midpoints.append(weed_mid)
        ...
        if if_draw:
            ...
            cv2.rectangle(image, (box[0], box[1] + box[3]), (box[0] + box[2], box[1]),
                          (50, 50, 128), 2, lineType=cv2.LINE_AA)
            cv2.putText(image, text='Weed', org=(box[0], box[1] - 5), fontFace=cv2.FONT_HERSHEY_SIMPLEX,
                        fontScale=0.6,
                        color=(50, 50, 128),
                        thickness=2,
                        lineType=cv2.LINE_AA)
...
# 去除苗或杂草
def separate_2dif(image, target_ayy, if_draw=True, if_weedpixs=False):
    ...
    for (x, y, w, h, cls) in target_ayy:
        ...
        cv2.rectangle(img=image,
                      ...
                      lineType=cv2.LINE_AA)
    if if_draw:
        img, weed_midpoints = findcontours_M(image, binary, if_draw=if_draw)
        for i in weed_midpoints_1:
            weed_midpoints.append(i)
        if if_weedpixs:
            green = cv2.countNonZero(binary)  # 计算二值化图像非零像素数量
            weed_pixs = green / (image.shape[0] * image.shape[1]) - 0.2#
            if weed_pixs <= 0.1:
                weed_pixs = 0.1
            elif 0.1 < weed_pixs <= 0.2:
                weed_pixs = 0.3
            else:
                weed_pixs = 0.6
            print(weed_pixs)
        return img, weed_midpoints, maize_midpoints, weed_pixs
...
def image_mosaic(big_img, area_img, area_axis, scale=0.1, if_scale=False, if_rgb=True):
    if if_rgb:
        try:
            ...
            area_axis[0]:(area_axis[0] + area_axis[3])] = area_img  # (H,W)
        except:
            return big_img
        ...
        big_img[area_axis[1]:area_axis[1] + area_axis[2], area_axis[0]:area_axis[0] + area_axis[3]] = area_img
        if if_scale:
            big_resized_img = rescaleFrame(big_img, scale=scale)
            return big_resized_img
        return big_img
...
def midpoints_plot(image, seeding_midpoints_arr, weed_midpoints_arr, if_weed=True, if_seeding=True):
    ...
    if if_weed:
        for (x, y) in weed_midpoints_arr:  # 绘制草点
            cv2.circle(mask, (int(x), int(y)), 6, color=(0, 0, 255), thickness=-1)
    if if_seeding:
