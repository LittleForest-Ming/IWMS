    ...
    for i, det in enumerate(pred):  # per image 每张图片
        ...
        if len(det):
            ...
            for *xyxy, conf, cls in reversed(det):
                ...
                All_Pre_labels.append((int(cls), xywh[0], xywh[1], xywh[0]+xywh[2], xywh[1]+xywh[3]))
    # 输出结果
    ...
    for file in dirs:
        if os.path.splitext(file)[1] == '.png':  # 只读取固定后缀的文件
            ...
            img, weed_midpoints, maize_midpoints, weed_pixs, if_Warning_Area = \
                tool_Differential_Decision_Making.separate_2dif(image, target_ayy, if_weedpixs=True)
            # 获取目标经纬度坐标
            ...
            if if_Warning_Area:
                heatmap_mask = tool_Differential_Decision_Making.create_img(int(image.shape[1]), int(image.shape[0]),
                                                                            if_rgb=False, dtype="int")
                heatmap_mask = ((heatmap_mask+weed_pixs+0.42) * 255).astype(np.uint8)
            ...
            mosaic_img = tool_Differential_Decision_Making.image_mosaic(big_img, mask_points,
            ...
            # cv2.waitKey(0)
            #############################################################
    return mosaic_img, mosaic_img_2, mosaic_img_3, Color_image
    # return heatmap_mask
    # return mosaic_img
    # return mosaic_img
...
if __name__ == '__main__':
    ...
    mosaic_image_1, mosaic_image_2, mosaic_image_3, heatmap_img = measure(dir_path, width=width, height=height)

    # 颜色区域映射矫正
    # # amend_color_img = np.array(heatmap_img / (np.max(heatmap_img) - np.min(heatmap_img)) * 255, dtype=np.uint8)
    # amend_color_img = np.array(heatmap_img / (np.max(heatmap_img) + 10) * 255, dtype=np.uint8)
    # # 处方图生成
